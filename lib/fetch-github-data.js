...
module.exports.searchUsers = async function searchUsers(props, page = 1) {
  try {
    ...
    const followers = 'followers:>=1000';
    const users = await github.search.users({
      //q: `${language}|${location}`,
      q: `${language}|${type}|${followers}`,
      sort: 'repositories',
      order: 'desc',
      per_page: 100,
      page,
...
module.exports.getReposByUser = async function getReposByUser(user, page = 1) {
  try {
    ...
    if (repos.meta.link && repos.meta.link.indexOf('next') !== -1) {
      ...
      if (nextPage > 1) return repos.data;
...
module.exports.getLanguageBytesByRepo = async function getLanguageBytesByRepo(repo, page = 1) {
  try {
    const languages = await github.repos.getLanguages({
    ...
    console.error(error);
    debug('Error fetching languages data from repo %s, page %s', repo.name, page);
    return {};
  }
};
